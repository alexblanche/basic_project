...
let edit (grid : bool) (m : bool array array) : unit =
	...
	let loop_previs (mbb : int) (i0 : int) (j0 : int) : int * int =
		let rec aux () =
			match Sdlevent.poll_event () with
				| Some (Mouse_Button_Up mbe) ->
					if mbe.mb_button = mbb
						then (mbe.mb_x, mbe.mb_y)
						else aux ()

				| Some (Mouse_Motion mme) ->
					(try
						let (i,j) = coord_to_cell mme.mm_x mme.mm_y in
						print_mat ren m grid instr;
						if !line
			...
			| Some (Mouse_Button_Down {mb_button = mbb; mb_x = x; mb_y = y}) ->
				if mbb = 1 then (* Left-click *)
					(try
						...
						let (ifi,jfi) = loop_previs 1 i0 j0 in
