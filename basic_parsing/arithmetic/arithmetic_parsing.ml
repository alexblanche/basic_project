...
let get_list_val (p : project_content) (a : int) (i : int) : complex =
  let (b, l) = p.list.(a) in
  if b
...
let get_mat_val (p : project_content) (a : int) (i1 : int) (i2 : int) : complex =
  let (b, m) = p.mat.(a) in
  if b
...
(* Returns the value of the variable of index i in the array var *)
let rec get_val (var : float array) (p : project_content) (n : basic_number) : complex =
  match n with
    | Value z -> z
    ...
    | Variable (ListIndex (a,e)) ->
      ...
      get_list_val p a (int_of_complex z))
    | Variable (MatIndex (a,e1,e2)) ->
      let z1 = eval var p e1 in
      let z2 = eval var p e2 in
      (if not ((is_int z1) && (is_int z2))
        then failwith "get_val: access to Mat from an index that is not an integer";
      get_mat_val p a (int_of_complex z1) (int_of_complex z2))
...
and calculate (var : float array) (p : project_content) (outq : basic_number list) (opq : arithm list) : complex =
  match outq, opq with
    (* Left parentheses left open are allowed in Casio Basic *)
    | _, Lpar::opqt -> calculate var p outq opqt
    | x2::x1::t, (Op o)::opqt -> calculate var p ((Value (apply_op o (get_val var p x1) (get_val var p x2)))::t) opqt
    | x::t, (Lunop lo)::opqt -> calculate var p ((Value (apply_lop lo (get_val var p x)))::t) opqt
    | [x], [] -> get_val var p x
...
and right_reduce var p output_q op_q =
  match (output_q,op_q) with
    ...
    | x2::x1::outq, (Op o)::opqt ->
      if left_assoc o
        then (output_q, op_q)
        else right_reduce var p ((Value (apply_op o (get_val var p x1) (get_val var p x2)))::outq) opqt
    | _, (Op o)::_ -> failwith ("reduce: Not enough operands for operator "^o)
    | x::outq, (Lunop lo)::opqt ->
        right_reduce var p ((Value (apply_lop lo (get_val var p x)))::outq) opqt
...
and shunting_yard (var : float array) (p : project_content)
  (lexlist : arithm list) (output_q : basic_number list) (op_q : arithm list) : complex =
  match (lexlist,op_q) with
    ...
    | (Runop ro)::t, _ ->
      (match output_q with
        | x::outq -> shunting_yard var p t ((Value (apply_rop ro (get_val var p x)))::outq) op_q
    ...
    | Comma::t, (Op o)::opqt ->
      if left_assoc o
        then (* Associative or left-associative *)
          (match output_q with
            | x2::x1::outq ->
              shunting_yard var p t ((Value (apply_op o (get_val var p x1) (get_val var p x2)))::outq) (Comma::opqt)
      ...
      if precedence o2 o1 = 1 || (precedence o2 o1 = 0 && left_assoc o1)
        then if left_assoc o2
          then
            (match output_q with
              | x2::x1::outq ->
                shunting_yard var p t ((Value (apply_op o2 (get_val var p x1) (get_val var p x2)))::outq) ((Op o1)::opqt)
    ...
    | Rpar::t, _::_ ->
      (match output_q, op_q with
          ...
          | x1::outq, Lpar::(Function fname)::opqt ->
            ...
            if af = 1
              then shunting_yard var p t ((Value (apply_func fname [get_val var p x1]))::outq) opqt
              else failwith ("Function "^fname^" has arity "^(string_of_int af)^", but receives 1 argument")
          | x2::x1::outq, Comma::Lpar::(Function fname)::opqt ->
            let af = arity fname in
            if af = 2
              then shunting_yard var p t ((Value (apply_func fname (List.map (get_val var p) [x1;x2])))::outq) opqt
          ...
          | x3::x2::x1::outq, Comma::Comma::Lpar::(Function fname)::opqt ->
            let af = arity fname in
            if af = 3
              then shunting_yard var p t ((Value (apply_func fname (List.map (get_val var p) [x1;x2;x3])))::outq) opqt
          ...
          | x4::x3::x2::x1::outq, Comma::Comma::Comma::Lpar::(Function fname)::opqt ->
            let af = arity fname in
            if af = 4
              then shunting_yard var p t ((Value (apply_func fname (List.map (get_val var p) [x1;x2;x3;x4])))::outq) opqt
          ...
          (* Operator evaluation *)
          | x::outq, (Lunop lo)::opqt ->
            shunting_yard var p (Rpar::t) ((Value (apply_lop lo (get_val var p x)))::outq) opqt
          | x2::x1::outq, (Op o)::opqt ->
            shunting_yard var p (Rpar::t) ((Value (apply_op o (get_val var p x1) (get_val var p x2)))::outq) opqt
...
(* var, p are the variable content array and the project_content containing the lists and matrices *)
and eval (var : float array) (p : project_content) (e : basic_expr) : complex =
  match e with
